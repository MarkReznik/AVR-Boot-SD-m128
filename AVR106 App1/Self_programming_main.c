...
unsigned char SdUpdateStatus()
{
    ...
    if(u8_update_file_flag)
    {
       if(u8_backup_file_flag)//in case "BACKUP" found check the "TEMPDATA" exists by mistake
       {
          if(u8_tempdata_file_flag)//if on some reason tempdata filename exists, then rename it 
          {              
