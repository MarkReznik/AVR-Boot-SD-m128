...
)
{
    ...
    res = pf_opendir(&dir, dirname);
    if (res == FR_OK) {
        i = strlen(oldname);//save old flename length
        for (;;) {
            ...
            for(j=0;j<i;j++){//check compare the oldname chars to filename read from directory
                if(fno.fname[j] != oldname[j]){
                    res=FR_NO_FILE;
                    break;//if not match -> read next filename in directory
                }
            }
            //on old filename part match, read the filenames sector to buffer[512] and replace the old filename with new then rewrite the buffer 
            if(i==j){
               //read sector on found filename
               res = disk_readp(rwbuf,dir.sect,0,512);
               if (res != FR_OK) break;
               i = strlen(newname);//save new flename length
               //replace the old name with new on accoding to index of filename in directory sector
               for(j=0;j<11;j++){
                  if(j<i){
                    rwbuf[((dir.index-1)*32)%512+j]=newname[j];
                  }
                  else{
                    rwbuf[((dir.index-1)*32)%512+j]=' ';//fill spaces rest of chars
                  }
               }
               //write the sector with new filename
       ...
       if (res != FR_OK) return res; 
        //open the dir and read/check the new filename is found
       res = pf_opendir(&dir, dirname);
       if (res != FR_OK) return res;//break;
       ...
       for(;;){
           ...
           if(i==j){
              break;//exit for loop on match read new filename 
           }
       }
    }
