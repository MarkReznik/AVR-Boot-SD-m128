...
unsigned char SdUpdateStatus()
{
    ...
    delay_ms(1000);        
#endif    
    if((u8_try_file_flag = FindFileName("", "TRY4",1))==0)
    {
    #ifdef DEBUG_LCD
        lcd_putsf("3");
        delay_ms(DEBUG_LCD_MSG_MS);        
    #endif
        if((u8_try_file_flag = FindFileName("", "TRY3",1))==0)
        {
        #ifdef DEBUG_LCD
            lcd_putsf("2");
            delay_ms(DEBUG_LCD_MSG_MS);        
        #endif            
            if((u8_try_file_flag = FindFileName("", "TRY2",1))==0)
            {
            #ifdef DEBUG_LCD
                lcd_putsf("1");
                delay_ms(DEBUG_LCD_MSG_MS);        
            #endif            
                if((u8_try_file_flag = FindFileName("", "TRY1",1))==0)//backup case
                {
                    ...
                    delay_ms(DEBUG_LCD_MSG_MS);        
                #endif            
                    u8_try_file_flag = FindFileName("", "TRY0",1);//factory case
                #endif
                }
                if(u8_try_file_flag)// in case backup or factory set u8_try_file_flag = 2;  to avoid rename update to backup
                   u8_try_file_flag = 2;
